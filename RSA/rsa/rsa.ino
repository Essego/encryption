...
void uint32_to_serial(uint32_t num);
uint32_t uint32_from_serial();
...
*/
void communication(uint32_t d, uint32_t n, uint32_t e, uint32_t m) {
    // Consume all early content from Serial to prevent garbage communication
    ...
    while (true) {
        ...
        if (Serial.available() >= 4) {
            ...
            uint32_t read = uint32_from_Serial();
        ...
        if (Serial.available() >= 1) {
            ...
            if ((int) byteRead == '\r') {
                ...
                Serial.print('\r');
                uint32_to_Serial(encrypt('\r', e, m));
                Serial.print('\n');
                uint32_to_Serial(encrypt('\n', e, m));
            } else {
                Serial.print(byteRead);
                uint32_to_Serial(encrypt(byteRead, e, m));
            }
        }
    }
}
...
/** Reads an uint32_t from Serial, starting from the least-significant
 * and finishing with the most significant byte.
 */
uint32_t uint32_from_Serial() {
    uint32_t num = 0;
    num = num | ((uint32_t) Serial.read()) << 0;
    num = num | ((uint32_t) Serial.read()) << 8;
    num = num | ((uint32_t) Serial.read()) << 16;
    num = num | ((uint32_t) Serial.read()) << 24;
    return num;
}
...
void handshakeServer(uint32_t& serverPrivateKey, uint32_t& smod, uint32_t& ckey, uint32_t& cmod) {
    ...
    while (state != S_DATA_EXCHANGE) {
        if (state == LISTEN) {
            if (Serial.available() >= 1 && Serial.read() == 'C') {
                state = WAITING_FOR_KEY;
            }
        }
        else if (state == WAITING_FOR_KEY) {
            if (wait_on_Serial(8, 1000)) {
                ckey = uint32_from_Serial();
                cmod = uint32_from_Serial();
                
                Serial.write('A');
                uint32_to_Serial(skey);
                uint32_to_Serial(smod);
...
void handshakeClient(uint32_t& clientPrivateKey, uint32_t& cmod, uint32_t& skey, uint32_t& smod) {
    ...
    while (state != C_DATA_EXCHANGE) {
        if (state == START) {
            Serial.write('C');
            uint32_to_Serial(ckey);
            uint32_to_Serial(cmod);
            state = WAITING_FOR_ACK;
        }
        else if (state == WAITING_FOR_ACK) {
            if (wait_on_Serial(9, 1000)) {
                if (Serial.read() == 'A') {
                    skey = uint32_from_Serial();
                    smod = uint32_from_Serial();
...
/** Waits for a certain number of bytes on Serial or timeout
